...
function load_game_scene_prep()
    ...
    ORDER_SIZE_TABLE = {2,1,1,1,1,1,1,1}  -- 每个load function的最大值
...
function order_load_game_scene_VFX(load_order)
    local switch = 
    {
        [1] = function()
            ...
            image_sprite_sheet_VFX_stage_game_scene_ground_smoke_horizontal_shot = 
            sprite_sheet_load(
                "asset/game_scene/VFX/stage_VFX/ground_smoke/horizontal_shot.json",
                love.graphics.newImage(ASSET_DATA[8]["stage_ground_smoke_horizontal_shot"])
            )
...
function preset_game_scene_training()
    ...
    init_character_anim_with(obj_char_game_scene_char_LP,anim_char_LP_5_stand_idle)
    init_character_anim_with(obj_char_game_scene_char_RP,anim_char_RP_5_stand_idle)
